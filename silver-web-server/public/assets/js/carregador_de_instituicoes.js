    ...
    if (xhr.status == 200 && xhr.readyState == 4) {
        ...
        if (Object.keys(resultadoFormatado).length > 0) {
            ...
            for (var i = 0; i < Object.keys(resultadoFormatado).length; i++) {
                ...
                alvoInstituicoes.innerHTML += `<tr><td>${alvoAtual.id}</td><td>${alvoAtual.nome}</td><td><button onclick=RedirecionarParaEdicao(${alvoAtual.id})>Editar</button></td><td><button onclick=IniciarExclusao(${alvoAtual.id})>Excluir</button></td><tr>`
            }
