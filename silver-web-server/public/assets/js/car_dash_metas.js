    ...
    if (xhr.status == 200 && xhr.readyState == 4) {
        ...
        if (Object.keys(resultadoFormatado).length > 0) {
            ...
            for (var i = 0; i < Object.keys(resultadoFormatado).length; i++) {
                ...
                alvoInstituicoes.innerHTML += `<tr><td scope="row">${titulo}</td><td>${formatada}</td><td>${batida}</td><td><button class="btn btn-primary" onclick=irParaPaginaDeEdicao(${alvoAtual.id_noticia})>Editar</button></td><td><button class="btn btn-danger" onclick=IniciarExclusao(${alvoAtual.id_meta})>Excluir</button></td><tr>`
